...
The datasets referenced in this document are stored in the project data folder and are processed by the notebook `Collecting_Data_EV_Charger_Demand (1).ipynb`. The information in this document was reviewed against the uploaded project files on **9 June 2026** and reflects the current dataset state after the updated traffic dataset was provided.

**Note:** Some datasets are raw public datasets downloaded from external open data sources, while others are project-generated outputs created through web scraping, preprocessing, or data integration. The updated traffic dataset is provided as a ZIP archive containing a CSV file.
...
| Hourly Traffic Counts | `road_traffic_counts_hourly_permanent.zip` / `road_traffic_counts_hourly_permanent.csv` | ZIP / CSV | Daily and hourly traffic volume counts from permanent stations | External open data |
...
|---|---|---|
| `nsw_ev_chargers.xlsx` | `Destination Chargers (AC)` | Existing destination AC charging stations in NSW. Current uploaded sheet contains approximately 1,011 records and 13 columns. |
...
| File Name | Sheet Name | Description |
|---|---|---|
| `nsw_ev_chargers.xlsx` | `Fast Chargers (DC)` | Existing DC fast charging stations in NSW. Current uploaded sheet contains approximately 252 clean records and 13 columns. |
...
| File Name | Sheet Name | Description |
|---|---|---|
| `nsw_ev_chargers.xlsx` | `Upcoming Fast Chargers` | Planned fast charging infrastructure in NSW. Current uploaded sheet contains approximately 187 records and 10 columns. |
...
| Project file | `road_traffic_counts_hourly_permanent.zip` |
| Extracted file | `road_traffic_counts_hourly_permanent.csv` |
| Current uploaded file size | 1,000,000 rows × 41 columns after extraction |
| Uploaded ZIP size | Approximately 67 MB |
| Extracted CSV size | Approximately 205 MB |
| Overall date range in updated upload | 2009-02-04 to 2025-12-31 |
| Modelling-window coverage | 2020-01-01 to 2025-06-30 is present |
| Rows within 2020-01-01 to 2025-06-30 | 619,930 rows across all classification values |
| Rows within 2020-01-01 to 2025-06-30 after `classification_seq` filter `[2, 3]` | 583,476 rows |
| Unique stations within modelling window after filter `[2, 3]` | 362 station keys |
...
| File Name | Description |
|---|---|
| `road_traffic_counts_hourly_permanent.zip` | Updated compressed traffic dataset uploaded to the project. It contains `road_traffic_counts_hourly_permanent.csv`. |
| `road_traffic_counts_hourly_permanent.csv` | Extracted permanent traffic count dataset containing daily total and hourly traffic counts by station, direction, classification, and date. |

**Updated note:** The new traffic file now covers the required project modelling window from **2020-01-01 to 2025-06-30**. The full file also includes records up to **2025-12-31**, but the current weather dataset ends on **2025-06-30**, so the final modelling merge should either filter traffic to **2025-06-30** or extend the weather data.
...
| 5 | Load permanent hourly traffic count records from the updated ZIP/CSV traffic dataset |
...
| 9 | Scrape or load Sydney weather history from Timeanddate.com |
...
| Traffic time window | Updated traffic file extends beyond the weather file end date | Filter traffic to the weather period or collect additional weather records |
...
| `road_traffic_counts_hourly_permanent.zip` | Updated traffic data now covers the required 2020-01-01 to 2025-06-30 modelling period; the full upload extends to 2025-12-31 | Use the extracted CSV and filter by the project modelling window before merging with weather |
...
3. The updated traffic file now includes the intended 2020-01-01 to 2025-06-30 modelling period. However, it also includes traffic records after 2025-06-30, while the weather file currently ends on 2025-06-30. The merge should therefore filter traffic to the available weather window or extend weather scraping to match the later traffic dates.
...
7. Hourly traffic fields contain missing values in some records. Aggregation should either use the `daily_total` column or apply a clear rule for handling incomplete hourly profiles.
...
| AAA EV Index Data Sources | [EV Index Data Sources](https://www.aaa.asn.au/research-data/electric-vehicle/ev-index-data-sources/) | Registration data provenance and source explanation |
