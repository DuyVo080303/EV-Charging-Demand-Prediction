...
GLOBAL_MODEL_PATH = "model_gru.keras"
GLOBAL_SCALER_PATH = "scaler_all.joblib"
GLOBAL_TAIL_PATH = "tail.npy"
...
def load_artifacts_for_cluster(geo_cluster: int):
    ...
    for cdir, mpath, spath, _ in tried:
        st.write(f"Tried: {cdir}, Model exists? {os.path.exists(mpath)}, Scaler exists? {os.path.exists(spath)}")
    raise FileNotFoundError(
        f"Không tìm thấy model/scaler cho cụm {geo_cluster}. "
        "Yêu cầu các file 'model_gru.keras' và 'scaler.joblib' trong một trong các thư mục: "
...
def forecast_with_exog(model, scaler, exog_scaled, seed_scaled, horizon):
    ...
    for t in range(horizon):
        exog_input = exog_scaled[t]  # Sử dụng từng dòng exog features
        next_input = np.append(seq[-seq_len:, 0], exog_input)  # Tạo đầu vào cho mô hình
        x = next_input.reshape(1, seq_len + 1, n_feat)  # reshape

        # Dự báo
        yhat_scaled = model.predict(x, verbose=0).ravel()[0]
        next_vec = np.empty((n_feat,), dtype=float)
        next_vec[0] = yhat_scaled  # Target (dự báo)
        next_vec[1:] = exog_input  # Exog (đã được scale)
